$(document).ready(function () {
    ...
    $('.analysis-extract-rule').click(function () {
        ...
        var data = {
            url: storageWebPage['StorageWebPage']['url'],
            html: storageWebPage['StorageWebPage']['content'],
            format: 'all'
        }
        $.post(target, data, function (results) {
            $("#pulsarHtmlWrapper").html(results['result']);
        }, 'json');
    });
...
//    $("#pulsarHtmlWrapper").load(target);
        $("#pulsarHtmlWrapper").removeClass('qiwur-hidden');
        $("#pulsarHtmlWrapper").fadeIn('slow');
...
//        ui.positon = $(this).data('ui-position');
        ...
        $(".extract-field-value .dropper").droppable({
            tolerance: 'touch',
            drop: function (event, ui) {
                if ($(ui.draggable).find('div').length * 2 > $('body').find('div').length) {
                    return;
                }
